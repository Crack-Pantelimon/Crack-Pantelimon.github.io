        ...
        if let Some(window) = web_sys::window() {
            if let Some(document) = window.document() {
                if let Ok(url_str) = document.url() {
                    ...
                    // Serve assets relative to the current deployment's path
                    // E.g. "https://crack-pantelimon.github.io/" or "https://username.github.io/repo/"
                    let url_no_hash = url_str.split('#').next().unwrap_or(&url_str);
                    let url_no_query = url_no_hash.split('?').next().unwrap_or(url_no_hash);
                    if let Some(last_slash_idx) = url_no_query.rfind('/') {
                        // Avoid splitting at the protocol slash (e.g. "https://")
                        if last_slash_idx > 8 {
                            return url_no_query[..=last_slash_idx].to_string();
                        }
                    }
                    let mut base = url_no_query.to_string();
                    if !base.ends_with('/') {
                        base.push('/');
                    }
                    return base;
